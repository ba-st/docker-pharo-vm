...
- Locate the zip file corresponding to the commit hash. It will have a name like
  `PharoVM-{{version}}-{{commit_hash}}-Linux-x86_64-bin.zip` (beware that multiple files with the same version might be available, we need the one with the commit equal to the one mentioned in the Pharo VM release)
